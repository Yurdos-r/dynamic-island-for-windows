...
function registerIpcHandlers() {
  registerIslandIpcHandlers({
    ...
    getUiSettings: () => ({ layout, systemMonitorEnabled }),
    onSystemRendererReady: () => {
    ...
    controlMedia: (action) => mediaController?.control(action) ?? { ok: false, available: false },
    seekMedia: (seconds) => mediaController?.control("seek", seconds) ?? { ok: false, available: false },
    writeClipboardText: (text) => clipboardMonitor?.writeText(text) ?? { ok: false, error: "Clipboard monitor is not available." },
    acceptClipboardPending: (id) => clipboardMonitor?.acceptPending(id) ?? { ok: false, error: "Clipboard monitor is not available." },
    dismissClipboardPending: (id) => clipboardMonitor?.dismissPending(id) ?? { ok: false, error: "Clipboard monitor is not available." },
    clearClipboardItems: () => clipboardMonitor?.clearItems() ?? { ok: false, error: "Clipboard monitor is not available." },
    removeClipboardItem: (id) => clipboardMonitor?.removeItem(id) ?? { ok: false, error: "Clipboard monitor is not available." }
  });
}
